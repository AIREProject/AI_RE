...
#include "../../OBI/Abilities/Public/AI_REPlayerGameplayAbility.h"
#include "AbilitySystem/Core/AIRECompanionGameplayTags.h"
#include "Misc/DataValidation.h"

bool UAI_REAbilitySetDataAsset::IsAbilitySetValid(FText& OutValidationError) const
{
	...
	for (const FAIRECompanionAbilitySetEntry& Entry : Abilities)
	{
		...
		if (!Entry.AbilityClass->IsChildOf(UAIRECompanionGameplayAbility::StaticClass()) && 
		    !Entry.AbilityClass->IsChildOf(UAI_REPlayerGameplayAbility::StaticClass()))
		{
			OutValidationError = NSLOCTEXT(
				"AIRECompanionAbilitySet",
				"InvalidAbilityBase",
				"Ability Set entries must derive from AIRECompanionGameplayAbility or AI_REPlayerGameplayAbility.");
			return false;
		}
...
#if WITH_EDITOR
EDataValidationResult UAI_REAbilitySetDataAsset::IsDataValid(FDataValidationContext& Context) const
{

(() => {
  ...
  const isPresenterView = viewParams.get("presenter") === "1";
  const isAudienceView = viewParams.get("audience") === "1";
  document.body.classList.toggle("is-presenter-view", isPresenterView);
  document.body.classList.toggle("is-audience-view", isAudienceView);
  ...
  const requestGameplayVideoFullscreen = () => {
    if (isPresenterView || isAudienceView) return;
  ...
  const updateSlides = ({ requestVideoFullscreen = false } = {}) => {
    ...
    document.dispatchEvent(new CustomEvent("aire:slidechange", {
      detail: {
        index: currentIndex,
        label: labelFor(slide),
  ...
  window.AIREPresentation = Object.freeze({
    slides,
    mainSlides,
    appendixSlides,
    gameplayVideo,
